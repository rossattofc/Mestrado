...
def draw_prediction():
    ...
    np.save(os.path.join(path,'rad-target'),np.array(y.detach().cpu()))
   
    np.save(os.path.join(path,'rad-prediction'),np.array(y_pred))
...
if __name__ == '__main__':
    ...
    if torch.cuda.is_available():
        model.load_state_dict(torch.load('PredRNN/Rad-PredRNN.pt'))
    else:
        state_dict = torch.load('PredRNN/Rad-PredRNN.pt',
